...
var player = {
  ...
  onlineHTML:function(id_target, poster, width, height, error, stream) {
    ...
    jQuery('#'+id_target).html(html);

      var video = jQuery('video:first').get(0);
      // video.load();
      video.play();
  },
  ...
  onlineHTML_:function(id_target, id_media, id_section, poster, width, height, online_onair) {
    ...
    jQuery.ajax({
      ...
      success: function(xml){
        jQuery(xml).find("flvserver").each(function(){
          if (jQuery(this).attr("id") == "live"+online_onair) {
            var balancer = jQuery(this).attr("balancer");
            var stream_name = jQuery(this).attr("stream_name");
            ...
            jQuery.ajax({
              ...
              success: function(data){
                ...
                jQuery('#'+id_target).html('<video src="'+onlinesrc+'" controls autoplay="autoplay" poster="' + poster + '" width=' + width +' height=' + height + '></video>');
              },
              error: function(){
                jQuery('#'+id_target).html('player initilization error 8');
              }
            });
          }
        });
      ...
      error: function(){
        jQuery('#'+id_target).html('player initilization error 9');
      }
    });
  },
  ...
  vodHTML:function(id_target, id_media, id_section, path, poster, width, height, server) {
    ...
    jQuery('#'+id_target).html('<video src="'+src+'" controls autoplay poster="' + poster + '" width=' + width +' height=' + height + '></video>');
  },
